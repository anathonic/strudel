...
>*4`).sound("sqr").slow(8).room(5).lpf(sine.range(2000, 2000).slow(16)).gain(0.2)

const drums1 = sound("bd ~ ~ bd ~ ~ bd ~ ~ ~ oh ~ hh ~ ~ ~ ")
.bank("ace").room(0.2).distort(0.3).lpf(8000).gain(0.8)

const drums2 = sound("sh tb sh tb sh tb sh tb sh tb sh tb sh tb sh tb sh").bank("d110").gain(0.1)

const allDrums = stack(drums1, drums2)

const pad = note(`< [a3] ~ [d3] ~ >`).sound("gm_fx_soundtrack").duration(2).gain(0.8)

const green = "#5FB147"
stack(
  arrange([16, synth]),
  arrange([8,silence],[16, drums1]),
  arrange([8,silence],[16,pad])
